...
struct overlay_session::impl
{
	...
	bool mark_id_dirty(const std::string& id)
	{
		if (id.empty()) return true;
		auto it = id_map.find(id);
		if (it == id_map.end() || !it->second) return false;
	...
	//    要素の自然サイズ (limits().min) が bounds を超えていれば、 その差分を
	//    左右/上下対称に足して覆う (どちら寄せで描かれるか分からないため)。
	//    変更前の見た目 (伸びていた頃) も覆う必要があるので、 呼出側は
	//    「変更前」と「変更後」の 2 回呼ぶ (合併されて両方を含む矩形になる)。
	...
	bool mark_element_dirty(ce::element& e)
	{
		...
		ce::rect b{};
		ce::extent nat{0, 0};
		if (!view->element_bounds(e, b, nat)) return false;
		const float d = (view_w > 0 && last_buf_w_ > 0)
			? static_cast<float>(last_buf_w_) / view_w : 1.0f;
		const float w = b.right - b.left;
		const float h = b.bottom - b.top;
		const float ex = nat.x > w ? (nat.x - w) : 0.0f;   // 横のはみ出し
		const float ey = nat.y > h ? (nat.y - h) : 0.0f;   // 縦のはみ出し
		const float m = 2.0f;   // AA / 端数の吸収
		mark_dirty_rect_px((b.left  - ex - m) * d, (b.top    - ey - m) * d,
...
bool overlay_session::update()
{
	...
	//   毎フレーム入れ替わり続ける (カーソルが 2 つ見えて画面内を
	...
	// 変化した id は新旧 2 つとも分かるので、 id_map から要素を引いて
	// **両方の矩形**をダーティにする (どちらか片方だと枠/hilite が消え残る)。
	// id 無しの要素へ移った場合など引けなければ全面へフォールバック。
	if (_impl->focused_id_slot &&
	    *_impl->focused_id_slot != _impl->dirty_last_focused) {
		const bool ok = _impl->mark_id_dirty(_impl->dirty_last_focused)
		              & _impl->mark_id_dirty(*_impl->focused_id_slot);
	...
	if (_impl->hovered_id_slot &&
	    *_impl->hovered_id_slot != _impl->dirty_last_hovered) {
		const bool ok = _impl->mark_id_dirty(_impl->dirty_last_hovered)
		              & _impl->mark_id_dirty(*_impl->hovered_id_slot);
...
void overlay_session::on_mouse_move(float sx, float sy, int mods)
{
	...
	// widget 自身の refresh(rect) で矩形として届くので、 移動イベント自体は
	// needs_render も立てない (widget 上を通らない移動・cursor-warp ナビの
...
bool overlay_session::on_key_down(ce::key_code key, int mods)
{
	...
	// 入力そのものでは needs_render を立てない。 見た目の変化は widget の
...
{
	if (!active()) return false;
	// 変化は refresh(rect) 経由で届く (on_key_down 参照)。
	ce::key_info ki{
		.key = key,
...
void overlay_session::on_pad_axis(ce::pad_axis axis, float value)
{
	...
	// 入力そのものでは needs_render を立てない。 focus 移動 / 値変化は
	// poll 内の処理が refresh / focus 変化として矩形ダーティになる
	// (on_key_down 参照)。 スティックは保持中毎フレーム軸値が届くので、
	// ここで立てると保持中ずっと無変化の全面再描画が続いてしまう。
