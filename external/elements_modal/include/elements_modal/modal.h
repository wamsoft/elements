...
//---------------------------------------------------------------------------
// テーマ (見た目の一括差し替え) — 仕様は docs/theming.md
//---------------------------------------------------------------------------

//! @brief 使用するテーマ名を実行時に指定する。 空文字で «指定なし»
//! (= app.jsonc の "theme" が使われる) に戻る。
//!
//! テーマは **画面を組むときに** 当たるので、 表示中の画面へ反映するには
//! 画面を組み直すこと (elements のテーマがプロセス全体で 1 つで、 一部の色は
//! スタイラの構築時に焼き込まれるため)。
void set_ui_theme(const std::string& name);

//! @brief set_ui_theme で指定されているテーマ名 (未指定なら空文字)。
...
//---------------------------------------------------------------------------
//  アトラスのデコードキャッシュ (プロセス全体で 1 つ)
//---------------------------------------------------------------------------
//
// アトラス画像は `path + scale` をキーにデコード済み pixmap をキャッシュして
// 使い回す。 画面を切り替えても手放さない設計 (長時間プレイでヒープが断片化
// したあと大きな連続領域が取れずデコードが失敗するのを避けるため) なので、
// **場面の切れ目でホストが明示的に落とす口**がこれ。
...
//! @brief キャッシュを budget まで切り詰める。 0 で «使われていないものを全部»。
//! @return 解放できたバイト数。
//! @note 予算そのものは変えない (一時的な切り詰め)。 **表示中の画面が使っている
//!       atlas は参照が残るので捨てられない** — 画面を閉じた後に呼ぶこと。
std::size_t trim_atlas_cache(std::size_t budget_bytes);

//! @brief 予算を恒久的に変更する。 0 でキャッシュ無効 (毎回デコード)。
//!        下げた場合はその場で切り詰める。
...
//---------------------------------------------------------------------------
// セッション共有変数 — 画面をまたいで保つ値
//
// 変数ストアは画面ごとに作り直されるので、 設定画面をタブで渡り歩くと
// «さっき動かしたスライダー» が既定値へ戻ってしまう。 画面 JSON が
//
//     "shared_vars": ["cfg_*", "ui_lang"]
//
// と宣言すると、 一致する変数は**セッション共有ストアと双方向**になる:
// 画面を組むとき共有側の値で初期化し (画面の "vars" 既定より優先)、
// 以後その変数が変わるたび共有側へ書き戻す。 パターンは完全一致か、
// 末尾 '*' の前方一致。
//
// 共有側へ行くのは «変化として書かれた» 値だけで、 widget の初期値
// (JSON の "initial" / "value") は出ない。 «誰も触っていない項目» が
// 最初に開いた画面の既定で固定されてしまうのを避けるため。 スライダーの
// display_var のように «組んだ時点で計算して書く» 値は共有側にも出る。
//
// 「どの値を持ち回るか」は画面が決める (ホストの実装は不要)。 ゲーム本体の
// セーブデータへ落とすのはホストの仕事なので、 下の API で読み書きできる。
//---------------------------------------------------------------------------

//! @brief セッション共有変数を書く (画面が未構築でも可)。
void set_shared_var(const std::string& name, const std::string& value);

//! @brief セッション共有変数の現在値をすべて返す (セーブ用)。
std::map<std::string, std::string> shared_vars();

//! @brief セッション共有変数を捨てる (タイトルへ戻る / ロード直前など)。
void clear_shared_vars();

//---------------------------------------------------------------------------
// 差し替え可能アトラス — «同じ画面・違う絵の束»
//
// CG 鑑賞のグループタブのように、画面はそのままで絵の束だけ入れ替えたい
// ことがある。 画面 JSON でアトラスを
//
//     "atlases": { "cg": { "path": "atlas/cg_g0.png", "swappable": true } }
//
// と宣言しておくと、 ホストが実行時に別の 1 枚へ差し替えられる。 widget は
// 作り直さないので、 レイアウトもフォーカスもそのまま保たれる。
//
// **差し替え先は同じ矩形割りであること**。 widget が持つ frames / rect は
// 変わらないので、 絵の位置がずれると別の絵が出る。 uitool の
// `tools/image_atlas.py --uniform` が «どのグループでも同じ格子» に詰める。
//
// swappable なアトラスはパス単位のキャッシュに乗らない (中身を書き換えると
// 同じ絵を使う他の画面を巻き添えにするため)。 その代わり画面をまたいだ
// 使い回しは効かない。
//---------------------------------------------------------------------------

//! @brief 名前付きアトラスの絵を差し替える。 path は画面の resource_base 起点。
//! @return 差し替えられたか (未宣言 / 読込失敗なら false、 表示は変わらない)。
bool set_atlas_image(const std::string& name, const std::string& path);

//! @brief いま差し替えられるアトラス名の一覧 (検証パネル / REPL 用)。
...
public:
	...
	//! @brief パッド軸ナビ (dpad/スティックによる focus 移動 + リピート) の
	//!        有効/無効。 複数セッションを重ねるホストは、 入力対象の最前面
	//!        セッションだけ true、 背面は false を毎フレーム設定する。 これに
	//!        より、 上に別画面を開いた瞬間に押しっぱなしだった方向キーで背面の
	//!        フォーカスが動き続ける現象を防ぐ。
	void set_pad_nav_active(bool on);
